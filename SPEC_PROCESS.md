...
- 2026-07-30，学生要求删除与课程交付清单无关的扩展，并确认“保守最小版”。
- 早期选择 Docker、Render 公网部署和后续对话 CLI，是开发过程中的范围扩张；这些决定保留在历史记录中，但不再属于最终规约。
- 最终分发选择老师允许的 CLI Release 链接：构建并上传 wheel。WebUI 继续保留为本地确定性 Mock 演示，不要求公网托管。
- 真实 OpenAI-compatible provider 与 Keyring 作为可选扩展保留；课程验收只依赖可注入 `ScriptedMockLLM`、自研 AgentLoop、三项机制演示和离线自动测试。
- `REFLECTION.md` 仍必须由学生本人撰写，智能体只可在用户明确请求后校对。
